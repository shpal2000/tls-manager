...
def zone_start_thread(node_info, c_args, z_index):
    ...
    requests.post('http://{}:8081/start'.format(zone_ipaddr)
                    , data = json.dumps({'cfg_file':'/rundir/traffic/cps1/config.json'
                                        , 'z_index' : z_index
                                        , 'netdev_list' : map (lambda n : n['container_iface'], node_info['networks']) })
...
if __name__ == '__main__':
    ...
    if cmd_args.cmd_name in ['cps', 'bw', 'tproxy', 'mcert']:
        ...
        if cmd_args.cmd_name in ['cps', 'bw', 'mcert']:
            cmd_args.na_iface_container = filter (lambda n : n['host_iface'] == cmd_args.na_iface, node_info['networks'])[0]
            cmd_args.nb_iface_container = filter (lambda n : n['host_iface'] == cmd_args.na_iface, node_info['networks'])[0]
            ...
            cmd_args.ta_iface_container = filter (lambda n : n['host_iface'] == cmd_args.ta_iface, node_info['networks'])[0]
            cmd_args.tb_iface_container = filter (lambda n : n['host_iface'] == cmd_args.tb_iface, node_info['networks'])[0]
        ...
        cfg_dir, result_dir = start_traffic(node_info, cmd_args, traffic_s)
